...
GitConfig::GitConfig(const std::string &gitDir) : gitDir(gitDir) {}
  ...
  try {
    ...
    if (std::filesystem::exists(configPath)) {
      ...
      if (!configFile.is_open()) {
        throw ConfigException("Failed to open config file for reading: " +
                              configPath);
      }
      std::string line;
      while (std::getline(configFile, line)) {
        ...
        if (pos != std::string::npos) {
          ...
          if (currentKey == key) {
            continue; // Skip lines with the same key
          }
        }
